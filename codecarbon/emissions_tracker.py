...
class BaseEmissionsTracker(ABC):
    ...
    def __init__(
        self,
        csv_write_interval: Optional[int] = _sentinel,
        ...
        self._set_from_conf(csv_write_interval, "csv_write_interval", -1, int) # -1 SOLO AL FINAL
    ...
    def _measure_power_and_energy(self) -> None:
        ...
        # ========== CSV and API Control ==========
        # Control específico para escritura CSV frecuente
        # =========================================
        if (
            self._csv_write_interval != -1
            and len(self._output_handlers) > 0
            and self._measure_occurrence % self._csv_write_interval == 0
        ):
            if 'emissions' not in locals():  # Si no se calculó arriba
                emissions = self._prepare_emissions_data()
                emissions_delta = self._compute_emissions_delta(emissions)
            
            # Escribir al CSV
            for handler in self._output_handlers:
                if hasattr(handler, 'out'):  # Solo handlers que soportan escritura CSV
                    handler.out(emissions, emissions_delta)
            
            logger.info(f"CSV record written at measurement {self._measure_occurrence}")
